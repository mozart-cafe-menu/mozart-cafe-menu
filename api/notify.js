...
async function getFCMEntries(projectId, accessToken) {
  ...
  return Object.entries(res.body)
    .filter(([, v]) => v && v.token)
    .map(([deviceId, v]) => ({ deviceId, token: v.token, lang: v.lang || 'el' }));
}
...
function buildBody(type, table, deviceLang, message) {
  if (type === 'message') {
    const lbl = MSG_LABEL[deviceLang] || 'Msg';
    return `${lbl} : ${message}`;
  }
  if (type === 'order') {
...
async function sendFCM(projectId, accessToken, entries, table, lang, type, title, message) {
  let sent = 0;
...
module.exports = async (req, res) => {
  ...
  try {
    const { table, lang, type, title, message } = req.body || {};
    ...
    const result = await sendFCM(sa.project_id, accessToken, entries, table, lang, type || null, title || null, message || null);
